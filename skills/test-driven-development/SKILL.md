...
**Verify RED:**
```bash
$ cargo test rejects_empty_prompt
...
mod tests {
    ...
    proptest! {
        #[test]
        fn never_panics_on_arbitrary_input(input in ".{0,1000}") {
            let _ = validate_prompt(&input);
        }
    }
}
```
...
```python
import pytest
from kratos_mcp.models import PromptInput
...
```bash
$ pytest -W error
FAILED tests/test_models.py::test_rejects_empty_prompt
```
